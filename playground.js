...
// Normalize challenge data without changing the existing puzzle flow.
// Some bundled/remote challenges arrive as ONE component containing the
// entire Java program (sometimes with literal \n sequences). When that
// happens, turn that single block into real, independently draggable pieces.
function normalizeChallengePieces(rawPieces){
  const pieces=Array.isArray(rawPieces)?rawPieces:[];
  if(pieces.length!==1) return pieces.map(p=>({...p,code:escapeLine(p?.code)}));

  const original=pieces[0] || {};
  let code=String(original.code ?? '').replace(/\r/g,'');
  const hasEscapedNewlines=/\\n/.test(code);
  if(hasEscapedNewlines && !code.includes('\n')) code=code.replace(/\\n/g,'\n');

  const lines=code.split('\n').filter(line=>line.trim()!=='');
  if(lines.length<=1) return [{...original,code}];

  return lines.map((line,index)=>({
    ...original,
    id:`${original.id || 'piece'}-${index+1}`,
    code:line
  }));
}
...
function getPieceId(piece){ return String(piece?.id ?? ''); }

function canonicalServerOrder(){ return state.canonicalPieces.map(p=>getPieceId(p)); }
...
function renderLanding(challenges){
  ...
  challenges.forEach(c=>{
    const key=c.unit_id||'UNIT_0';
    if(!grouped.has(key)) grouped.set(key,{id:key,title:c.unit_title||'Java Playground',division:c.unit_division||'',order:Number(c.unit_order??99),challenges:[]});
    grouped.get(key).challenges.push(c);
  });
  const units=[...grouped.values()].sort((a,b)=>a.order-b.order);
  root.innerHTML=`
    ...
    <div class="playground-toolbar">
      <div class="playground-toolbar-left"><span class="badge badge-cyan">🧩 ${challenges.length} Challenges</span><span class="badge badge-amber">☕ Java</span><span class="badge badge-purple">📚 ${units.length} Tracks</span></div>
      <span class="playground-card-number">Select a unit to begin</span>
    </div>
    <div class="unit-list" id="unit-list"></div>`;
  const list=$('#unit-list');
  if(!units.length){ list.innerHTML='<div class="playground-empty">No active Playground challenges are available yet.</div>'; return; }
  list.innerHTML=units.map((unit,unitIndex)=>{
    const items=[...unit.challenges].sort((a,b)=>Number(a.challenge_number)-Number(b.challenge_number));
    return `<section class="playground-unit-section" data-unit="${esc(unit.id)}">
      <div class="playground-unit-header">
        <div class="playground-unit-heading"><div class="playground-unit-number">${unit.id==='WIPRO'?'WIPRO':`UNIT ${unitIndex+1}`}</div><div><h2>${esc(unit.title)}</h2><p>${esc(unit.division||'Programming challenges')}</p></div></div>
        <div class="playground-unit-stats"><span class="badge badge-cyan">${items.length} Challenges</span><span class="badge badge-amber">Java</span></div>
      </div>
      <div class="playground-grid unit-challenge-grid">
        ${items.map((c,i)=>`<button class="playground-card" type="button" data-id="${esc(c.id)}" aria-label="Open ${esc(c.title)}">
          <div class="playground-card-top"><span class="playground-card-number">${unit.id==='WIPRO'?'Wipro':`Unit ${unitIndex+1}`} · Challenge ${String(i+1).padStart(2,'0')}</span><span class="badge badge-${c.difficulty==='Beginner'?'green':c.difficulty==='Advanced'?'crimson':'amber'}">${esc(c.difficulty||'Challenge')}</span></div>
...
async function checkSolution(){
  ...
  try{
    const local = state.challenge?.isLocalWipro
      ? { correct: state.placed.every(Boolean) && state.placed.map(getPieceId).join('|') === state.canonicalPieces.map(getPieceId).join('|'), code: assembledCode() }
      : flexibleSolutionCheck();
    if(!local.correct){
      const messages={
...
function getLocalWiproChallenges(){
  return Array.isArray(window.WIPRO_PLAYGROUND_CHALLENGES)
    ? window.WIPRO_PLAYGROUND_CHALLENGES.map(c => ({...c, isLocalWipro:true}))
    : [];
}

function mergePlaygroundChallenges(remote){
  const local=getLocalWiproChallenges();
  const remoteIds=new Set((remote||[]).map(c=>String(c.id)));
  return [...(remote||[]), ...local.filter(c=>!remoteIds.has(String(c.id)))];
}

function getCombinedChallengeIds(remote){
  const all=mergePlaygroundChallenges(remote).sort((a,b)=>{
    const unitDiff=Number(a.unit_order??99)-Number(b.unit_order??99);
    if(unitDiff) return unitDiff;
    return Number(a.challenge_number??0)-Number(b.challenge_number??0);
  });
...
async function loadChallenge(id){
  ...
  const local=getLocalWiproChallenges().find(c=>String(c.id)===String(id));
  if(local){ renderChallenge(local); return; }
  const {data,error}=await supabase.from('playground_public_challenges').select('id,title,question,difficulty,category,language,pieces,points,challenge_number,unit_id,unit_title,unit_division,unit_order').eq('id',id).maybeSingle();
...
async function loadLanding(){
  ...
  const {data,error}=await supabase.from('playground_public_challenges').select('id,title,question,difficulty,category,language,points,challenge_number,unit_id,unit_title,unit_division,unit_order').eq('is_active',true).order('challenge_number',{ascending:true});
  if(error){ console.warn('[Playground] Remote challenges unavailable; showing local Wipro content.',error); renderLanding(getLocalWiproChallenges()); return; }
  renderLanding(mergePlaygroundChallenges(data||[]));
}
function bindChallengeEvents(){
  ...
  $('#next-challenge')?.addEventListener('click',async()=>{ const {data}=await supabase.from('playground_public_challenges').select('id,challenge_number,unit_id,unit_order').eq('is_active',true).order('challenge_number',{ascending:true}); const ids=getCombinedChallengeIds(data||[]), idx=ids.indexOf(String(state.challenge.id)), next=ids[idx+1]||ids[0]; if(next) location.href=`playground.html?id=${encodeURIComponent(next)}`; });
}
