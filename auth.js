...
async function redirectHome() {
    ...
    try {
        ...
        const destination = admin ? 'admin.html' : 'home.html';
        console.info(`[Auth] Authenticated user routing → ${destination}`);
        window.location.replace(new URL(destination, window.location.href).href);
    } catch (roleError) {
        console.error('[Auth] Role check failed; using student fallback:', roleError);
