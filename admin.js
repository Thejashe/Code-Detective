...
const HARD_CASE_TOTAL = 72;
const PACK_CASE_TOTAL = 13;
const TOTAL_CASE_CATALOG = LEGACY_CASE_TOTAL + HARD_CASE_TOTAL + PACK_CASE_TOTAL;

const state = {
  users: [],
  selected: null,
  totalCases: TOTAL_CASE_CATALOG,
  assignments: [],
  broadcasts: [],
  notifications: [],
  atRisk: [],
...
async function sendBroadcast(){
  ...
  const type = $('#broadcast-priority')?.value || 'general';
  const target = $('#broadcast-target')?.value || 'Students';
  ...
  // Resolve recipients ONLY from the selected audience.
  // For "Selected", checkbox values are the student's Supabase user_id.
  const recipients =
    target === 'Selected'
      ? [...document.querySelectorAll('[data-broadcast-recipient]:checked')]
  ...
  if(!sender_id || !session?.access_token){
    alert('Your admin session has expired. Please sign in again.');
    return;
  }

  const sendButton =
    document.querySelector('#broadcast-form button[type="submit"]');
  ...
  try{
    ...
    /*
     * STEP 1
     * Save the notification in Supabase.
     *
     * One notification row is created for each selected user.
     */
     ...
     *
     * IMPORTANT:
     * We pass the EXACT SAME Supabase user IDs selected
     * in the admin panel.
     *
     * The send-push Edge Function is responsible for:
     *
     * user_id
     *    ↓
     * device_push_tokens
    ...
    /*
     * STEP 3
     * Read the Edge Function response.
     */
    const pushBody =
      await pushResponse
        .json()
     ...
     * don't pretend the push was successful.
     */
    if(!pushResponse.ok){
      ...
      throw new Error(
        pushBody?.error ||
        pushBody?.reason ||
        `Push service returned HTTP ${pushResponse.status}.`
      );
    }
    ...
    /*
     * STEP 4
     * Read delivery statistics returned by send-push.
     */
    const sent =
      Number(pushBody?.sent || 0);
    ...
    const failed =
      Number(pushBody?.failed || 0);


    /*
     * STEP 5
     * Store broadcast history locally.
     */
    state.broadcasts = [
      {
        title,
    ...
    /*
     * STEP 7
     * Show the REAL delivery result.
     */
    if(sent === 0){

      alert(
        'The notification record was saved, but no selected user has a registered Android push device.'
      );

    }else if(failed > 0){

      alert(
        `Push sent to ${sent} of ${total} registered device(s). ` +
        `${failed} delivery attempt(s) failed. ` +
        `The in-app notification was saved for all ` +
        `${uniqueRecipients.length} selected recipient(s).`
      );

    }else{

      alert(
        `Notification sent to ${sent} registered device(s) ` +
        `for ${uniqueRecipients.length} selected recipient(s).`
      );
    }
